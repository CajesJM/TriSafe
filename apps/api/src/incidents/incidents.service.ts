...
export class IncidentsService {
  ...
  async createDraft(passengerId: string, dto: DraftIncidentDto) {
    if (dto.rideId) {
      const ride = await this.prisma.ride.findFirst({
        where: { id: dto.rideId, passengerId },
      });
      if (!ride)
        throw new NotFoundException("Ride not found for this passenger");
      ...
      if (existing)
        throw new ConflictException(
          "This ride already has an incident report.",
        );
    }
    if (
      dto.evidenceData &&
      dataImageByteLength(dto.evidenceData) > 2 * 1024 * 1024
    )
      throw new ForbiddenException("Evidence image must be 2 MB or smaller.");
    const suggestion = this.ai.draft(dto.rawDescription);
    const incidentId = await this.allocateIncidentId();
    try {
      return await this.prisma.incident
        .create({
          data: {
            ...
            passengerId,
            rideId: dto.rideId,
            rawDescription: dto.rawDescription.trim(),
            aiDraft: suggestion.draft,
            category: dto.category ?? suggestion.category,
            evidence: dto.evidenceData
              ? {
                  create: {
                    fileName:
                      dto.evidenceName?.trim() || "incident-evidence.jpg",
                    mimeType: dto.evidenceData.substring(
                      5,
                      dto.evidenceData.indexOf(";"),
                    ),
        ...
        .then((incident) => ({
          ...
          aiNotice:
            "AI only assists with drafting and categorization. The LGU makes all review decisions.",
        }));
    } catch (error) {
      if (
  ...
  async submit(passengerId: string, id: string, dto: SubmitIncidentDto) {
    ...
    if (incident.status !== IncidentStatus.DRAFT)
      throw new ForbiddenException("Only draft incidents can be submitted");
  ...
  async updateDraft(
    ...
    if (!incident) throw new NotFoundException("Incident not found");
    if (incident.status !== IncidentStatus.DRAFT)
      throw new ForbiddenException("Only draft incidents can be edited");
    if (
      dto.evidenceData &&
      dataImageByteLength(dto.evidenceData) > 2 * 1024 * 1024
    )
  ...
  async review(actorId: string, id: string, dto: ReviewIncidentDto) {
    ...
    if (!incident) throw new NotFoundException("Incident not found");
    const reviewed = await this.prisma.incident.update({
      where: { id },
      data: {
        status: dto.status,
    ...
    await this.audit.record({
      ...
      entityType: "Incident",
      entityId: id,
      details: { status: dto.status, category: dto.category },
    });
    return reviewed;
  }

  private async allocateIncidentId() {
    ...
    throw new ConflictException(
      "Unable to allocate a unique incident report ID. Please try again.",
    );
  }
}

function dataImageByteLength(value: string) {
  const base64 = value.split(",").at(-1) ?? "";
