...
export class BoholLocationService {
  ...
  async streetSuggestions(
    ...
    const hierarchy = await this.requireHierarchy(
      municipalityCode,
      barangayCode,
    );
    return this.searchPhoton(
      cleanQuery,
  ...
  async validateRegistrationAddress(dto: BoholAddressDto) {
    if (
      dto.provinceCode !== BOHOL_CODE ||
      dto.provinceName.toLowerCase() !== "bohol"
    ) {
      throw new BadRequestException(
        "Driver registration currently supports Bohol addresses only.",
      );
    }
    const hierarchy = await this.requireHierarchy(
    ...
    if (
      hierarchy.municipality.name.localeCompare(
        dto.municipalityName,
        undefined,
        { sensitivity: "accent" },
      ) !== 0 ||
      hierarchy.barangay.name.localeCompare(dto.barangayName, undefined, {
        sensitivity: "accent",
      }) !== 0
    ) {
      throw new BadRequestException(
        "The selected municipality and barangay names do not match their PSGC codes.",
      );
    }
    ...
    if (!selected) {
      throw new BadRequestException(
        "Select a verified Street/Purok suggestion before registering the driver.",
      );
    }
    if (selected.postalCode !== dto.postalCode) {
      throw new BadRequestException(
        "The postal code does not match the selected Bohol location.",
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        "The selected Street/Purok coordinates could not be verified.",
      );
    }
    return {
  ...
  async validateDriverPresentAddress(dto: DriverPresentAddressDto) {
    if (
      ...
      throw new BadRequestException(
        "Driver addresses are currently limited to Bohol.",
      );
    }
    const hierarchy = await this.requireHierarchy(
      dto.municipalityCode,
    ...
    if (
      ...
      throw new BadRequestException(
        "The municipality and barangay do not match their official PSGC records.",
      );
    }
    return {
      provinceCode: BOHOL_CODE,
  ...
  private async requireMunicipality(code: string) {
    const item = (await this.municipalities()).find(
      (entry) => entry.code === code,
    );
    if (!item)
      throw new BadRequestException(
        "Select a valid Bohol municipality or city.",
      );
    return item;
  }

  private async requireHierarchy(
    municipalityCode: string,
    barangayCode: string,
  ) {
    const municipality = await this.requireMunicipality(municipalityCode);
    ...
    if (!barangay)
      throw new BadRequestException(
        "Select a barangay that belongs to the chosen municipality.",
      );
  ...
  private async searchPhoton(
    ...
    url.searchParams.set(
      "q",
      `${query}, ${barangay}, ${municipality}, Bohol, Philippines`,
    );
    ...
    const response = await this.fetchJson<{ features?: PhotonFeature[] }>(
      url.toString(),
    );
  ...
  private async fetchJson<T>(url: string): Promise<T> {
    try {
      ...
      throw new ServiceUnavailableException(
        "The Philippine location directory is temporarily unavailable.",
      );
    }
  }
}
