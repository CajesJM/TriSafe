      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (vehicle != null) ...[
            ...
            Text(
                '${vehicle.vehicleType.replaceAll('_', ' ')} · ${vehicle.plateNumber}'),
            Text(vehicle.franchiseNumber == null
                ? 'No franchise record'
                : 'Franchise ${vehicle.franchiseNumber}'),
            const SizedBox(height: 14),
            Wrap(spacing: 8, runSpacing: 8, children: [
              _StatusChip(
                  label: 'Transport: ${_label(result.transportStatus)}',
                  color: presentation.color),
              _StatusChip(
                  label: 'QR: ${_label(result.qrStatus)}',
                  color: result.qrStatus == 'ACTIVE'
                      ? const Color(0xff337418)
                      : Colors.red.shade700),
              if (result.accountStatus != null)
                _StatusChip(
                    label: 'Account: ${_label(result.accountStatus!)}',
                    color: result.accountStatus == 'ACTIVE'
                        ? const Color(0xff337418)
                        : Colors.red.shade700),
            ]),
          ],
...
({String title, IconData icon, Color color, Color background}) _presentation(
    QrVerificationResult result) {
  if (!result.legitimate) {
    return (
      title: 'Not an LGU-issued QR',
      icon: Icons.gpp_bad_outlined,
      color: const Color(0xffa52323),
      background: const Color(0xffffeeee)
    );
  }
  if (result.qrStatus == 'REVOKED') {
    return (
  ...
  switch (result.transportStatus) {
    ...
    default:
      return (
        title: 'Verified by TriSafe',
        icon: Icons.verified,
        color: const Color(0xff337418),
        background: const Color(0xffeaf7e4)
      );
  }
}
