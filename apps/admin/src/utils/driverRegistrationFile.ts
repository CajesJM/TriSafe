...
export const driverFileFormats: { value: DriverFileFormat; label: string; extension: string }[] = [
...
export function createDriverRegistrationFileData(driver: Driver): DriverRegistrationFileData {
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { label: "Email address", value: driver.email ?? "Not recorded" },
      ...
      {
        title: "License and transport eligibility",
        fields: [
          { label: "License number", value: driver.licenseNumber },
          { label: "License renewal", value: formatDate(driver.renewalDate) },
      ...
      },
      {
        ...
        fields: [
          ...
          { label: "Franchise issued", value: formatDate(driver.franchise?.issuedAt) },
          { label: "Franchise expiration", value: formatDate(driver.franchise?.expiresAt) },
          { label: "Franchise status", value: driver.franchise?.status ?? "Not assigned" },
        ],
      },
      {
        title: "Vehicle and QR identity",
        fields: [
          { label: "Plate number", value: vehicle?.plateNumber ?? "Not assigned" },
          { label: "Vehicle type", value: vehicle?.vehicleType?.replaceAll("_", " ") ?? "Not assigned" },
          { label: "LGU QR status", value: vehicle?.qrCode?.token ? "Generated" : "Not generated" },
        ],
      },
    ],
  };
...
export function downloadDriverRegistrationFile(driver: Driver, format: DriverFileFormat) {
  ...
  downloadBlob(createDriverRegistrationFileBlob(data, format), `${baseName}${extension}`);
}

export function createDriverRegistrationFileBlob(data: DriverRegistrationFileData, format: DriverFileFormat) {
...
function createPdf(data: DriverRegistrationFileData) {
  ...
  const text = (value: string, x: number, y: number, size = 9, bold = false, color = "0.125 0.125 0.125") => {
    commands.push(`BT /${bold ? "F2" : "F1"} ${size} Tf ${color} rg ${x} ${y} Td (${pdfEscape(value)}) Tj ET`);
  };
  ...
  text(`Generated ${formatDateTime(data.generatedAt)} from live registry data`, 42, 745, 8, false, "0.4 0.4 0.4");
  commands.push("0.933 0.973 0.914 rg 42 695 511 36 re f");
  text("CURRENT DATABASE RECORD", 54, 717, 8, true, "0.137 0.357 0.067");
  text("Temporary passwords are excluded because TriSafe stores secure password hashes only.", 54, 704, 7, false, "0.25 0.35 0.22");
  let y = 671;
  ...
  text("Confirm current account and transport eligibility through the live TriSafe registry.", 42, 32, 7, false, "0.42 0.42 0.42");
...
function createDocx(data: DriverRegistrationFileData) {
  const sectionXml = data.sections.map((section) => `
  ...
  const files = [
    ...
    { name: "docProps/app.xml", data: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>TriSafe</Application></Properties>` },
  ];
...
function zipBlob(files: { name: string; data: string }[]) {
  ...
  return new Blob([...localParts.map(toArrayBuffer), ...centralParts.map(toArrayBuffer), toArrayBuffer(end)], { type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" });
}

function writeZipHeader(view: DataView, signature: number, crc: number, size: number, nameLength: number) {
...
function formatDateTime(value: string) { return new Date(value).toLocaleString("en-PH", { dateStyle: "medium", timeStyle: "short" }); }
function safeFileName(value: string) { return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""); }
function shorten(value: string, max: number) { const text = toAscii(value); return text.length > max ? `${text.slice(0, max - 3)}...` : text; }
function toAscii(value: string) { return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x20-\x7e]/g, "?"); }
function pdfEscape(value: string) { return toAscii(value).replace(/([\\()])/g, "\\$1"); }
