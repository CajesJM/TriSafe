...
export function parsePersonName(fullName: string): PersonNameParts {
  ...
  if (normalized.includes(",")) {
    ...
    return {
      lastName,
      firstName: initialMatch
        ? givenNames.slice(0, initialMatch.index).trim()
        : givenNames,
      middleInitial: initialMatch?.[1]?.toUpperCase() ?? "",
    };
  }
  const words = normalized.split(" ").filter(Boolean);
  if (words.length <= 1)
    return { lastName: words[0] ?? "", firstName: "", middleInitial: "" };
  return {
    lastName: words.at(-1) ?? "",
...
export function validatePersonName(parts: PersonNameParts) {
  ...
  if (
    lastName.length < 2 ||
    lastName.length > 45 ||
    !namePartPattern.test(lastName)
  )
    return "Enter a valid last name using letters, spaces, apostrophes, or hyphens.";
  if (
    firstName.length < 2 ||
    firstName.length > 45 ||
  ...
  if (parts.middleInitial && !/^\p{L}$/u.test(parts.middleInitial))
    return "Middle initial must contain only one letter.";
  return "";
}
