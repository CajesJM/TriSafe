    ...
    <div className="dashboard-page">
        ...
        <article className="card chart-card">
          <div className="panel-title-row">
            <div>
              ...
              <h3>Rides started in the last 7 days</h3>
              <p>Live sessions recorded by the passenger application.</p>
            </div>
            <span className="section-count">
              {dashboard.rideActivity.reduce((sum, day) => sum + day.count, 0)}{" "}
              rides
            </span>
          </div>
          <div
            className="bar-chart"
            role="img"
            aria-label="Rides started in the last seven days"
          >
            {dashboard.rideActivity.map((day) => (
              <div className="bar-column" key={day.date}>
                <div className="bar-value">{day.count}</div>
                <div className="bar-track">
                  <i
                    style={{
                      height: `${Math.max(day.count ? 10 : 2, (day.count / maxActivity) * 100)}%`,
                    }}
                  />
                </div>
                <span>{day.label}</span>
        ...
        <article className="card action-card">
          <div className="panel-title-row">
            <div>
              ...
              <h3>LGU work queue</h3>
              <p>Move directly to records that need attention.</p>
            </div>
          </div>
          <ActionItem
            icon="!"
          ...
          <ActionItem
            icon="₱"
            label="Maintain fare rules"
            detail="Official fare matrix"
            onClick={() => onNavigate("fares")}
          />
          <ActionItem
            icon="✉"
            label="Notify verified drivers"
            detail="Send announcement"
      ...
      <section className="dashboard-secondary-grid">
        <article className="card breakdown-card">
          <div className="panel-title-row">
            <div>
              <p className="eyebrow">ACCOUNT DISTRIBUTION</p>
              <h3>Users by role</h3>
            </div>
            <button
        ...
        <article className="card breakdown-card">
          <div className="panel-title-row">
            <div>
              <p className="eyebrow">RIDE OUTCOMES</p>
              <h3>All recorded rides</h3>
            </div>
            <strong className="panel-total">{dashboard.rides.total}</strong>
          </div>
          <RoleBar
            label="Completed"
      ...
      <section className="card recent-registry">
        <div className="panel-title-row">
          <div>
            <p className="eyebrow">DRIVER REGISTRY</p>
            <h3>Recently registered drivers</h3>
            <p>Latest approved operator and vehicle records.</p>
          </div>
...
function ActionItem({
  ...
  return (
    <button className="action-item" onClick={onClick} type="button">
      <span className={urgent ? "urgent" : ""}>{icon}</span>
      <div>
        <b>{label}</b>
        <small>{detail}</small>
      </div>
