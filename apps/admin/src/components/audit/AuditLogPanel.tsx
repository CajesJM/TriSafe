...
export function AuditLogPanel({ logs }: { logs: AuditLog[] }) {
  ...
  const filtered = useMemo(
    () =>
      logs.filter((log) => {
        const text =
          `${log.action} ${log.entityType} ${log.entityId ?? ""} ${log.actorName ?? ""} ${log.actorEmail ?? ""}`.toLowerCase();
        return (
  ...
  return (
    <section className="card data-card audit-panel">
      <div className="section-heading">
        <div>
          <span className="eyebrow">ACCOUNTABILITY</span>
          <h3>Administrative activity</h3>
          <p className="section-description">
            A read-only record of LGU registry, fare, communication, and safety
            decisions.
          </p>
        </div>
        <span className="section-count">Latest {logs.length}</span>
      </div>
      <DataToolbar
        search={search}
        onSearch={updateSearch}
        searchLabel="Search action, actor, or record ID"
        filter={entity}
        onFilter={updateEntity}
        ...
        <EmptyState
          ...
          text={
            logs.length
              ...
              : "Administrative actions will appear here as they are completed."
          }
        />
      ) : (
              ...
              <div className="audit-event-main">
                <div className="audit-event-heading">
                  <div>
                    ...
                    <span className="entity-chip">{log.entityType}</span>
                  </div>
                ...
                <p>
                  {log.actorName
                    ...
                    : "TriSafe system"}
                  {log.entityId ? ` · Record ${shortId(log.entityId)}` : ""}
                </p>
                {log.details && (
...
function iconFor(action: string) {
  if (action.includes("DRIVER")) return "D";
  if (action.includes("FARE")) return "₱";
